%% ============================================================
%  new_anova_heatingrate_v1.m
%  ANOVA + Tukey pipeline voor HEATING RATE
%  Zelfde structuur als new_anova_v8.m (TempRise)
%  ============================================================
...
outDir = 'ANOVA_HeatingRate_FiberPower';
if ~exist(outDir, 'dir')
    mkdir(outDir);
end
...
requiredCols = {'fiberID','power_mW','freq_Hz','pulse_ms','dutyCycle_pct','heating_rate_20s'};
...
data.HeatingRate = data.heating_rate_20s;
...
mdl = fitlm(data, 'HeatingRate ~ Fiber*Power + PulseSetting');
...
writetable(anova_table, fullfile(outDir, 'ANOVA_HeatingRate_FiberPowerPulse.csv'));
...
[p_fiber, tbl_fiber, stats_fiber] = anova1(data.HeatingRate, data.Fiber, 'off');
...
[p_power, tbl_power, stats_power] = anova1(data.HeatingRate, data.Power, 'off');
...
[p_inter, tbl_inter, stats_inter] = anova1(data.HeatingRate, interaction_group, 'off');
...
summary_file = fopen(fullfile(outDir, 'ANOVA_HeatingRate_Summary.txt'),'w');

fprintf(summary_file, "=== ANOVA HeatingRate: Fiber * Power + PulseSetting ===\n\n");
