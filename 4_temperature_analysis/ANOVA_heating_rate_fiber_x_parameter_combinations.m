%% ============================================================
%  new_anova_heatingrate_parameters_v1.m
%  ANOVA: HeatingRate ~ Fiber * PulseSetting + Power (controle)
%  Identiek aan new_anova_parameters_v1.m maar met HeatingRate
%  ============================================================
...
outDir = 'ANOVA_HeatingRate_ParameterCombinations';
if ~exist(outDir, 'dir')
    mkdir(outDir);
end
...
mdl = fitlm(data, 'HeatingRate ~ Fiber * PulseSetting + Power');
...
[p_ps, tbl_ps, stats_ps] = anova1(data.HeatingRate, data.PulseSetting, 'off');
...
fprintf(summary_file, "=== ANOVA HeatingRate: Fiber * PulseSetting + Power (controle) ===\n\n");
...
for i = 1:height(T_fiber)
    if T_fiber.pValue(i) < 0.05
        fprintf(summary_file, "%s vs %s: diff = %.4f, p = %.3g   <<< SIGNIFICANT\n", ...
            T_fiber.Fiber1{i}, T_fiber.Fiber2{i}, T_fiber.Diff(i), T_fiber.pValue(i));
    else
        fprintf(summary_file, "%s vs %s: diff = %.4f, p = %.3g\n", ...
            T_fiber.Fiber1{i}, T_fiber.Fiber2{i}, T_fiber.Diff(i), T_fiber.pValue(i));
    end
end
